...
* **O Problema Resolvido:** O objetivo traçado no Milestone 1 foi o de desenvolver um modelo preditivo capaz de identificar transações fraudulentas num cenário de extremo desequilíbrio (apenas 0,17% de fraudes), minimizando as perdas financeiras sem causar atrito aos clientes. O critério SMART, que exigia o cumprimento simultâneo de três métricas no conjunto de teste, foi plenamente alcançado pelo modelo XGBoost final: AUPRC de 0.8251 (≥ 0.80), Recall de 78,95% (≥ 75%) e Precision de 93,75% (≥ 85%). A robustez destes resultados foi adicionalmente validada por Stratified K-Fold Cross-Validation (K=5), com Recall médio de 85,98% (± 0.04) e AUPRC média de 0.8131 (± 0.03), confirmando que o desempenho não depende de uma divisão específica dos dados.
...
| **PI 4** (SMOTE vs. scale_pos_weight) | O SMOTE não melhorou; o tratamento nativo via _scale_pos_weight_ foi superior. |
